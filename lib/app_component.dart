...
@Component(
  ...
  directives: const [materialDirectives, MaterialAutoSuggestInputComponent, DadataDirective, NgIf],
...
class AppComponent {
  ...
  SelectionModel addressSelectionModel = new SelectionModel.withList(allowMulti: false);
  /// Обработка события выбора элемента списка подсказок.
  address_blur() {
    cityFiasId = addressSelectionModel.selectedValues.first['data']['city_fias_id'];
    cityKladrId = addressSelectionModel.selectedValues.first['data']['city_kladr_id'];
    street = addressSelectionModel.selectedValues.first['data']['street'];
    streetTypeFull = addressSelectionModel.selectedValues.first['data']['street_type_full'];
  }

  /// Пустой список. Необходим для инициализации.
  SelectionOptions fioOptions = new SelectionOptions([new OptionGroup<List<int>>(const <List<int>>[]) ]);
  String fioInputText;
  final SelectionModel surnameSelectionModel = new SelectionModel.withList(allowMulti: false);

  /// Контроллер выбора подсказки в поле Фамилия.
  surname_blur() {    setGender(surnameSelectionModel);  }

  /// Имя
  String nameInputText;
  SelectionOptions nameOptions = new SelectionOptions([new OptionGroup<List<int>>(const <List<int>>[]) ]);
  final SelectionModel nameSelectionModel = new SelectionModel.withList(allowMulti: false);
  ...
  /// БАНК.
  /// Пустой список. Необходим для инициализации.
  SelectionOptions bankOptions = new SelectionOptions([new OptionGroup<List<int>>(const <List<int>>[]) ]);
  String bankInputText;
  /// Геттер класса реализации отрисовки элементов списка.
  ComponentRenderer get bankComponentRenderer =>  (_) => OneRowBankRendererComponent;
  final SelectionModel bankSelectionModel = new SelectionModel.withList(allowMulti: false);
  Bank bank = null;
  ...
  /// ОРГАНИЗАЦИЯ
  ///
  SelectionOptions jurOptions = new SelectionOptions([new OptionGroup<List<int>>(const <List<int>>[]) ]);
  String jurInputText;
  /// Геттер класса реализации отрисовки элементов списка.
  ComponentRenderer get jurComponentRenderer =>  (_) => OneRowJurRendererComponent;
  final SelectionModel jurSelectionModel = new SelectionModel.withList(allowMulti: false);
  ...
  ///  EMAIL
  SelectionOptions emailOptions = new SelectionOptions([new OptionGroup<List<int>>(const <List<int>>[]) ]);
...
@Component(
  ...
  styles: const ['.bank { border-bottom: 1px solid gray;} .second_row { font-size: 12px; color: gray;}']
)
class OneRowBankRendererComponent implements RendersValue {
  ...
  set value(addressMap) {
    ...
    bankCity = ( addressMap['data']['address']['data'] != null ) ? addressMap['data']['address']['data']['city'] : '';
  }
}
...
@Component(
    ...
    styles: const ['.jur { border-bottom: 1px solid gray;} .second_row { font-size: 12px; color: gray;}']
)
class OneRowJurRendererComponent implements RendersValue {
  ...
  set value(jurMap) {
    ...
    inn = jurMap['data']['inn'] ?? '';
    jurCity = ( jurMap['data']['address']['data'] != null ) ? jurMap['data']['address']['data']['city'] : '';
  }
}